...
const DesitinationMap: React.FC<Props> = ({
	...
	const ClickMarker = () => {
		useMapEvents({
			click: async (e) => {
				...
				if (middleFlag === -1) {
					...
					relayPoint.splice(middleFlag, 0, {
						Point: e.latlng,
						Relay: true,
					});
			...
			relayPoint.map((elem, index) => {
				if (!elem.Relay || true) {
                    ...
                    <g fill="white" stroke="white" stroke-width="1">
                        <text text-anchor="middle" x="15" y="25">${
							elem.Relay ? '' : ++counter.value
						}</text>
                    </g>
                    </svg>`,
						iconSize: [30, 45],
		...
		return <React.Fragment>{markerMaker()}</React.Fragment>;
	};
						...
						eventHandlers={{
							click: async (e) => {
								...
								if (isok) {
									...
									const firstindex = relayPoint.reduce(
										(previous, current, index) =>
											current.Point.equals(first)
												? index
												: previous,
										-1
									);
									const lastindex = relayPoint.reduce(
										(previous, current, index) =>
											current.Point.equals(last)
												? index
												: previous,
										-1
									);
