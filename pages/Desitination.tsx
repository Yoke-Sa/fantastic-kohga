...
const Desitination: NextPage = () => {
	...
	const allButtons = (disabled: boolean) => {
		if (isAfterRouteSearch) {
			...
			(
				document.getElementById('routingButton') as HTMLButtonElement
			).disabled = disabled;
			(document.getElementById('junkai') as HTMLInputElement).disabled =
				disabled;
			...
			(
				document.getElementById('beforeTutorial') as HTMLButtonElement
			).disabled = disabled;
			(
				document.getElementById('beforeReset') as HTMLButtonElement
			).disabled = disabled;
			(
				document.getElementById('beforeBack') as HTMLButtonElement
			).disabled = disabled;
			(
				document.getElementById('beforejunkai') as HTMLInputElement
			).disabled = disabled;
		}
	};
	...
	const onClickBackPage = () => {
		setPoly([]);
		setRelayPoint(
			relayPoint.reduce((previous, current) => {
				if (!current.Relay) {
	...
	const onClickMiddlePoint = async () => {
		...
		const { headTrue } = relayPoint.reduce(
			(prev, cur) => {
