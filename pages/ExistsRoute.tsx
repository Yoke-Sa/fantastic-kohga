...
const ExistsPage: NextPage = () => {
	...
	const FirstPost = async () => {
		...
		try {
			...
			if (result.succeeded) {
				if (result.passableNames !== undefined)
					setPassbleNameList(result.passableNames);
			...
			<div className="exist-page">
				...
				<header>
					<select
						...
						{passbleNameList
							? passbleNameList.map((routename, index) => (
									<option
										value={routename.routeName}
										key={index}
										disabled={!routename.available}>
										{routename.routeName}
									</option>
							  ))
							: null}
